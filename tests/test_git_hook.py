...
class TestHookExecution(unittest.TestCase):
    ...
    def test_hook_appends_record_with_env_session_id(self):
        ...
        # The hook stores whatever `git rev-parse --show-toplevel` returns,
        # which resolves OS-level symlinks (notably /tmp -> /private/tmp on
        # macOS). Compare against the resolved repo path.
        self.assertEqual(rec["repo"], str(Path(self.repo).resolve()))
...
class TestInstallGitHookCommand(unittest.TestCase):
    ...
    def test_install_per_repo_drops_hook_and_sets_config(self):
        ...
        # cli.cmd_install_git_hook derives the hooksPath from
        # `git rev-parse --show-toplevel`, which resolves OS-level symlinks
        # (notably /tmp -> /private/tmp on macOS). Compare against the
        # resolved repo path so the test is symlink-agnostic.
        expected = str(Path(self.repo).resolve() / ".claude-usage-hooks")
